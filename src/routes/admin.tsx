...
import { store, useStore, formatNGN, type Food, type Category, type Status, type OrderStatus, type SoupType, type PaymentStatus } from "@/lib/store";
...
type Tab = "foods" | "orders" | "users" | "payments";
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="flex flex-wrap gap-2 mb-6 border-b">
        {(["foods", "orders", "users", "payments"] as Tab[]).map((t) => (
          <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 text-sm font-semibold capitalize border-b-2 -mb-px ${tab === t ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}>{t}</button>
        ))}
      </div>
          ...
          <div key={f.id} className="rounded-xl border bg-card overflow-hidden">
            ...
            <div className="p-4">
              ...
              <div className="mt-3 flex gap-2">
                <select value={f.status} onChange={(e) => store.updateFood(f.id, { status: e.target.value as Status })} className="flex-1 rounded-md border bg-background px-2 py-1.5 text-xs">
                ...
                <button onClick={() => { if (confirm(`Delete ${f.name}?`)) { store.deleteFood(f.id); toast.success("Deleted"); } }} className="rounded-md border p-2 hover:bg-destructive hover:text-destructive-foreground"><Trash2 className="h-3.5 w-3.5" /></button>
              </div>
            </div>
          </div>
        ))}
...
function FoodForm({ initial, onClose }: { initial: Food | null; onClose: () => void }) {
  const [f, setF] = useState<Omit<Food, "id">>(initial ?? {
    name: "", image: "", price: 0, description: "", category: "Foods", status: "Available", isSwallow: false, dailyLimit: 10,
  });

  const setCat = (category: Category) => setF({ ...f, category, dailyLimit: category === "Foods" ? 10 : 3, isSwallow: category === "Foods" ? f.isSwallow : false });

  const save = (e: React.FormEvent) => {
    e.preventDefault();
    if (!f.name || !f.image || f.price <= 0) return toast.error("Fill all required fields");
    if (initial) { store.updateFood(initial.id, f); toast.success("Updated"); }
    else { store.addFood(f); toast.success("Added"); }
    onClose();
  };

  return (
    <div className="fixed inset-0 z-50 bg-foreground/50 backdrop-blur flex items-center justify-center p-4" onClick={onClose}>
      <form onClick={(e) => e.stopPropagation()} onSubmit={save} className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-xl bg-card p-6 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field l="Category">
            <select className="input" value={f.category} onChange={(e) => setCat(e.target.value as Category)}>
...
function OrdersTab() {
  ...
  const filtered = useMemo(() => orders.filter((o) => {
    if (fStudent && o.userId !== fStudent) return false;
    if (fHostel && o.hostel !== fHostel) return false;
    if (fCat) {
      const cats = o.items.map((i) => foods.find((f) => f.id === i.foodId)?.category);
      if (!cats.includes(fCat as Category)) return false;
    }
    return true;
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-xl border bg-card p-12 text-center text-muted-foreground">No orders match.</div>
      ) : (
              ...
              <div key={o.id} className="rounded-xl border bg-card p-4">
                <div className="flex flex-wrap justify-between gap-3">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <select value={o.status} onChange={(e) => store.updateOrderStatus(o.id, e.target.value as OrderStatus)} className="rounded-md border bg-background px-2 py-1.5 text-xs">
...
function PaymentsTab() {
  ...
  const setStatus = (id: string, status: PaymentStatus) => { store.updatePaymentStatus(id, status); toast.success(`Marked ${status}`); };
  ...
  return (
    <div>
      ...
      {pending.length === 0 ? (
        <p className="text-sm text-muted-foreground mb-6">Nothing awaiting verification.</p>
      ) : (
        <div className="space-y-2 mb-6">
          {pending.map((o) => {
            const u = users.find((x) => x.id === o.userId);
            return (
              <div key={o.id} className="rounded-lg border bg-card p-3 flex flex-wrap justify-between gap-3 items-center">
                <div>
