...
function Home() {
    ...
    if (!user)
        return null;
    const filtered = useMemo(() => foods.filter((f) => (cat === "All" || f.category === cat) && f.name.toLowerCase().includes(q.toLowerCase())), [foods, cat, q, orders]);
    return (<main className="mx-auto max-w-6xl px-4 py-8">
      <section className="rounded-3xl bg-gradient-to-br from-primary via-primary/90 to-[oklch(0.55_0.18_30)] p-8 md:p-12 text-primary-foreground mb-8 relative overflow-hidden">
        ...
        <div className="relative">
          <div className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1 text-xs font-medium backdrop-blur"><Flame className="h-3.5 w-3.5"/> Fresh today</div>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div className="flex gap-2">
          {["All", "Foods", "Protein"].map((c) => (<button key={c} onClick={() => setCat(c)} className={`rounded-full px-4 py-2 text-sm font-medium transition ${cat === c ? "bg-foreground text-background" : "bg-secondary text-secondary-foreground hover:bg-accent"}`}>{c}</button>))}
        </div>
        <div className="relative">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        {filtered.length === 0 && <div className="col-span-full text-center text-muted-foreground py-12">No meals found.</div>}
      </div>
    </main>);
}
function FoodCard({ food, left }) {
    ...
    const add = () => {
        if (disabled)
            return toast.error(left <= 0 ? "Daily limit reached" : `Currently ${food.status}`);
        store.addToCart(food.id, 1, food.isSwallow ? soup : undefined);
        toast.success(`${food.name} added to cart`);
    };
    const statusColor = food.status === "Available" ? "bg-success/15 text-success" : food.status === "Preparing" ? "bg-warning/20 text-warning-foreground" : "bg-destructive/15 text-destructive";
    return (<article className="group overflow-hidden rounded-2xl border bg-card transition hover:shadow-lg hover:-translate-y-0.5">
      <div className="relative aspect-[4/3] overflow-hidden bg-secondary">
        <img src={food.image} alt={food.name} className="h-full w-full object-cover transition group-hover:scale-105" loading="lazy"/>
      ...
      <div className="p-4">
        ...
        {food.isSwallow && (<div className="mt-3">
