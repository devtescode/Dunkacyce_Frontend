    ...
    <div className="grid lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 space-y-3">
        {options.map((o) => {
          ...
          return (<button key={o.id} onClick={() => setMethod(o.id)} className={`w-full text-left flex gap-4 rounded-xl border-2 p-4 transition ${active ? "border-primary bg-primary/5" : "border-border hover:border-primary/40"}`}>
        ...
        {/* {method === "Moniepoint" && (<div className="rounded-xl border border-dashed bg-warning/10 p-4 text-sm">
          ...
          <p className="mt-2 font-mono">Moniepoint · <strong>5648857264</strong> · Dunn-kayce Management - Catering Setrvices - Dunnkayce Kitchen</p>
      ...
      <aside className="rounded-xl border bg-card p-5 h-fit space-y-3">
        ...
        <ul className="text-sm space-y-1 border-b pb-3">
          {items.map((i) => (<li key={i.foodId + (i.soupType ?? "")} className="flex justify-between gap-2">
